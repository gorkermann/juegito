...
static void unijoy_sysfs_add_button(struct unijoy_inph_source *, int, int, int);
static void unijoy_sysfs_del_button(int);
static void unijoy_sysfs_add_axis(struct unijoy_inph_source *, int, int, int);
...
static ssize_t unijoy_sysfs_show(struct kobject *kobj, struct attribute *attr, 
  ...
  offset += scnprintf(buf+offset, PAGE_SIZE-offset,
                      "\n# type dev src dst inverted? status\n");
  
  for (i = 0; i < output.buttons_total; i++) {
    if (output.source_buttons_map[i].device_id == ULLONG_MAX)
      continue;
    offset += scnprintf(buf+offset, PAGE_SIZE-offset,
                      "map, BTN, %5llu, %3d, %3d, %s, %s\n",
                      output.source_buttons_map[i].device_id,
                      output.source_buttons_map[i].index, 
                      i,
                      (output.source_buttons_map[i].invert ? "yes" : "no"),
                      unijoy_inph_mapping_names[
                        (output.source_buttons_map[i].source ? 0 : 1)
                      ]);
  }
  for (i = 0; i < output.axis_total; i++) {
    if (output.source_axis_map[i].device_id == ULLONG_MAX)
      continue;
    offset += scnprintf(buf+offset, PAGE_SIZE-offset,
                      "map, AXS, %5llu, %3d, %3d, %s, %s\n",
                      output.source_axis_map[i].device_id,
                      output.source_axis_map[i].index, 
                      i,
                      (output.source_axis_map[i].invert ? "yes" : "no"),
                      unijoy_inph_mapping_names[
...
#define OPWORDTEST(word, outcome)  \
  ...
  switch (op) {
    ...
    case 3:
      ...
      source = unijoy_sysfs_find(id);
      unijoy_sysfs_add_button(source, arg1, arg2, 0);
      break;
    ...
    // add_axis
    case 5:
      sscanf(ptr, "%llu %d %d %d", &id, &arg1, &arg2, &arg3);
      source = unijoy_sysfs_find(id);

      // if arg3 is -1, axis is inverted
      if (arg3 != -1) {
      //  arg3 = 0;
      }

      unijoy_sysfs_add_axis(source, arg1, arg2, arg3);
      break;
...
static struct unijoy_inph_source *unijoy_sysfs_find(__u64 id) {
  ...
  if (!result) {
    printk(KERN_ALERT "unijoy: no source found with id %d\n", id);
  }
...
#define UNIJOY_ADD_RESOURCE(single, name, MAX_VALUE) \
  static void unijoy_sysfs_add_ ## single (struct unijoy_inph_source *source, \
                                           int src_no, int dst_no, int invert) { \
    int i; \
    if (!source) \
      return; \
    ...
    if (dst_no < 0) { \
      for (i = 0; i < output. name ## _total ; i++) { \
        if (output.source_ ## name ## _map[i].device_id == ULLONG_MAX) { \
          dst_no = i; \
          break; \
        } \
      } \
    ...
    output.source_ ## name ## _map[dst_no].index  = src_no; \
    output.source_ ## name ## _map[dst_no].device_id = source->id; \
    output.source_ ## name ## _map[dst_no].invert = invert; \
  }
...
#define UNIJOY_DEL_RESOURCE(single, name) \
  static void unijoy_sysfs_del_ ## single (int dst_no) { \
    ...
    if (dst_no < 0 || dst_no >= output. name ## _total) \
      return; \
    if (output.source_ ## name ##_map[dst_no].device_id == ULLONG_MAX) \
      return; \
    output.source_ ## name ## _map[dst_no].source = 0; \
    output.source_ ## name ## _map[dst_no].device_id = ULLONG_MAX; \
    if (dst_no+1 == output. name ## _total) { \
      i = dst_no; \
      while (output.source_ ## name ## _map[i].device_id == ULLONG_MAX) \
        i--; \
...
#define CLEAN_RESOURCE(name) \
  do { \
    for (i = 0; i < output. name ## _total; i++) { \
      if (output.source_ ## name ## _map[i].device_id == source->id) { \
        output.source_ ## name ## _map[i].source = 0; \
        if (forever) \
          output.source_ ## name ## _map[i].device_id = ULLONG_MAX; \
      } \
    } \
    i--; \
    while (output.source_ ## name ## _map[i].device_id == ULLONG_MAX) \
...
static struct unijoy_inph_source *unijoy_inph_create(struct input_dev *dev,
  ...
  printk(KERN_ALERT "unijoy: unijoy_inph_create()\n");
...
static void unijoy_inph_relink(struct unijoy_inph_source *source, __u64 id) {
  int i;
  for (i = 0; i < output.buttons_total; i++) {
    if (output.source_buttons_map[i].device_id == id) {
      output.source_buttons_map[i].source = source;
    }
  }
  for (i = 0; i < output.axis_total; i++) {
    if (output.source_axis_map[i].device_id == id) {
...
static int unijoy_inph_connect(struct input_handler *handler,
  ...
  if (strlen(dev->dev.kobj.name) < 6) {
    printk(KERN_ALERT "unijoy: cannot id device (name too short): %s %s,\n", dev->name, dev->dev.kobj.name);  
    return 0;
  }
  ...
  // old part with hw-derived id (doesn't work for two of the same device?)
  old_id = ((long)dev->id.bustype << 48)
...
static int unijoy_thread(void *nulldata) {
  ...
  printk(KERN_ALERT "unijoy: started thread\n");	
  ...
  switch (type) {
    case EV_KEY:
      if (code < BTN_MISC || value == 2)
        return;
      event_index = source->button_map[code - BTN_MISC];
      for (i = 0; i < output.buttons_total; i++) {
        if (output.source_buttons_map[i].source == source &&
            output.source_buttons_map[i].index == event_index) {
      ...
      break;
    case EV_ABS:
      event_index = source->axis_map[code];

     // value = unijoy_inph_correct(value, &source->corrections[event_index]);
     // printk(KERN_ALERT "unijoy: corrected: %d\n", value);
      for (i = 0; i < output.axis_total; i++) {        
        if (output.source_axis_map[i].source == source &&
            output.source_axis_map[i].index == event_index) {

          // change value if axis is inverted
          if (output.source_axis_map[i].invert && 
              handle->dev && 
              handle->dev->absinfo) {
            value = handle->dev->absinfo[event_index].maximum - value;
          }
...
static void unijoy_inph_register(void) {
  ...
  if (output.axis_total > 0) {
    ...
    for (i = 0; i < output.axis_total; i++) {
      ...
      if (idev->absinfo) {
        idev->absinfo[i].value = 128;
        idev->absinfo[i].minimum = 0;
        idev->absinfo[i].maximum = 255;
        idev->absinfo[i].flat = 15;
      } else {
        printk(KERN_ALERT "could not allocate absinfo\n");
      }
    }
  }
...
int __init unijoy_init(void) {
  ...
  printk(KERN_ALERT "unijoy_init()\n");
	
  error = unijoy_sysfs_setup();

  if (error) {
    printk(KERN_ALERT "unijoy: an error occurred\n");	
    return error;
  }
  ...
  if (error) {
    printk(KERN_ALERT "unijoy: input_register_handler(): an error occurred: %d\n", error); 
    goto err_free_sysfs; 
  }

  for (i = 0; i < ABS_CNT; i++) {
    output.source_axis_map[i].device_id = ULLONG_MAX;
  }
  for (i = 0; i < UNIJOY_MAX_BUTTONS; i++) {
    output.source_buttons_map[i].device_id = ULLONG_MAX;
  }
